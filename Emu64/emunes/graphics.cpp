...
olc::Sprite& graphics::GetScreen()
{
	return sprScreen;
}

olc::Sprite& graphics::GetPatternTable(uint8_t i, uint8_t palette)
{
...
olc::Pixel& graphics::GetColourFromPaletteRam(uint8_t palette, uint8_t pixel)
{
	return palScreen[ppuRead(0x3F00 + (palette << 2) + pixel) & 0x3F];
}

olc::Sprite& graphics::GetNameTable(uint8_t i)
{
	return sprNameTable[i];
}

uint8_t graphics::cpuRead(uint16_t addr, bool rdonly)
{
	uint8_t data = 0x00;
...
void graphics::cpuWrite(uint16_t addr, uint8_t data)
{
	switch (addr)
	{
	case 0x0000:
...
uint8_t graphics::ppuRead(uint16_t addr, bool rdonly)
{
	uint8_t data = 0x00;
	addr &= 0x3FFF;
...
void graphics::ppuWrite(uint16_t addr, uint8_t data)
{
	addr &= 0x3FFF;
...
void graphics::ConnectCartridge(const std::shared_ptr<cartridge>& cartridge)
{
	this->cart = cartridge;
}
